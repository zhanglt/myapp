...
"拆分范围不能大于文档总页数"); return;
    ...
    out = out + "_" + parts[0].toStdString() + "-" + parts[1].toStdString();
...
// 第二次拆分
splitPdf(tempDir.toStdString() + "/" + filename + "_split.pdf", out,
subPages); QFile tempfile(tempDir + "/" + QString::fromStdString(filename) +
"_split.pdf"); tempfile.remove(); QMessageBox::information(nullptr,
"PDF拆分完成！", "文件保存在保存：" + ui->lineEditSplitOutput->text());
...
void MainWindow::on_pushButton_clicked() {
  // 设置字体和文本
  QFont font("宋体", ui->lineEdit_fs->text().toInt());

  QStringList textLines;
  // textLines <<
  // "南昌大学第二附属医院红角洲分院二期、红角洲分院三期等建设项目智";
  QString t = ui->textEdit_text->toPlainText();
  textLines = t.split("\n");
...
*/
  foreach (const QString &line, textLines) {
    int lineWidth = getTextWidth(line, ui->cBoxFont->currentText(),
                                 ui->lineEdit_fs->text().toInt())
                        .cx;
    if (lineWidth > maxWidth) {
      maxWidth = lineWidth;
    }
  ...
  // 旋转角度（45度）
  // 将角度转换为弧度
  qreal angle = ui->lineEdit->text().toDouble();  //旋转角度
  qreal radians = qDegreesToRadians(angle);       //旋转弧度
  ...
  int x3 = w;
  int y3 = 0;
  int x11 = (x1 - x0) * qCos(radians) + (y1 - y0) * qSin(radians) + x0;
  int y11 = (y1 - y0) * qCos(radians) - (x1 - x0) * qSin(radians) + y0;
  int x22 = (x2 - x0) * qCos(radians) + (y2 - y0) * qSin(radians) + x0;
  int y22 = (y2 - y0) * qCos(radians) - (x2 - x0) * qSin(radians) + y0;
  int x33 = (x3 - x0) * qCos(radians) + (y3 - y0) * qSin(radians) + x0;
  int y33 = (y3 - y0) * qCos(radians) - (x3 - x0) * qSin(radians) + y0;
  ...
  if (((angle >= 0) && (angle <= 90)) || ((angle >= 180) && (angle <= 270))) {
    xMax = x22;
    yMax = qAbs(y11) + qAbs(y33);
    qDebug() << "旋转角度" << angle;
    qDebug() << "X最大值" << xMax;
    qDebug() << "Y最大值" << yMax;
  } else {
    xMax = (qAbs(x11) + qAbs(x33));
    yMax = y22;
    qDebug() << "旋转角度" << angle;
    qDebug() << "X最大值" << xMax;
    qDebug() << "Y最大值" << yMax;
  ...
  // 调整fontMetrics.horizontalAdvance(line) 带来的测量误差
  /*
  int cols = xMax / fontMetrics.height();
...
*/
  xMax = 595;
  yMax = 841;
  QSize rotatedTextBlockSize(xMax + 100,
                             yMax);  //当文字旋转时，高度要做相关调整

  QSvgGenerator generator;
  generator.setFileName("c:/test.svg");
  generator.setSize(rotatedTextBlockSize);              // 设置大小
  generator.setViewBox(QRect(0, 0, xMax + 100, yMax));  // 视口大小
  ...
  /*
  QColor shadowColor;
  shadowColor = QColor(0, 120, 215, 100);  //阴影颜色设置
  painter.setRenderHint(QPainter::HighQualityAntialiasing);
  // 绘制背景
  painter.fillRect(0, 0, xMax + 100, yMax,
                   shadowColor);  // 使用阴影颜色填充整个画布
*/
  // 绘制其他内容
  ...
  for (int i = 0; i < textLines.size(); ++i) {
    QPoint textPosition(0,
                        0 + i * lineHeight);  //当文字旋转时，纵坐标要做相关调整
    painter.drawText(
  ...
  test("c:/1.pdf", "c:/test.pdf", "asdfasdfasd", ui->lineEdit_x->text(),
       ui->lineEdit_y->text(), ui->lineEdit_lx->text(), ui->lineEdit_ly->text(),
       ui->lineEdit_w->text(), ui->lineEdit_h->text());
  this->open(QUrl::fromLocalFile("c:/test.pdf"));

  const char *filename = "c:/test.svg";
  int width, height;

  getSVGDimensions(filename, width, height);
  qDebug() << "SVG Dimensions: " << width << " x " << height;
}
using namespace std;
void MainWindow::on_pushButton_2_clicked() {
  SIZE size =
      getTextWidth(ui->lineEditWaterText->text(), ui->cBoxFont->currentText(),
                   ui->lineEdit_fs->text().toInt());
  qDebug() << "字体：" << ui->cBoxFont->currentText() << endl;
  qDebug() << "文本宽度为：" << size.cx << "像素" << endl;
  qDebug() << "文本高度为：" << size.cy << "像素" << endl;
}
